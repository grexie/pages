...
export class HeadContext extends EventEmitter {
  ...
  #mutateChildList(mutation: MutationRecord) {
    ...
    for (let i = 0; i < mutation.removedNodes.length; i++) {
      Array.from(document.head.childNodes)
        .find(node => node.isEqualNode(mutation.removedNodes[i]))
        ?.remove();
    }
    ...
    for (let i = 0; i < mutation.addedNodes.length; i++) {
      if (
        Array.from(document.head.childNodes).find(node =>
          node.isEqualNode(mutation.addedNodes[i])
        )
      ) {
        continue;
      }
        ...
        children = (
          <>
            {Children.map(
              ...
              (child, i) => handleElement(child, i)
            )}
          </>
        );
      } else if (isElement(props.children)) {
        children = handleElement(props.children as ReactElement);
      } else {
