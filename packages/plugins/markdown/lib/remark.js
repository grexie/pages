...
const extensions = ['yml', 'yaml', 'json', 'js', 'jsx', 'cjs', 'cjsx', 'mjs', 'mjsx', 'ts', 'tsx', 'cts', 'ctsx', 'mts', 'mtsx'];
 ...
 */
export const remarkPages = ({
  name
} = {}) => (ast, file) => {
  const pagesFiles = glob.sync('**/*.pages.{' + extensions.join(',') + '}', {
    cwd: process.cwd(),
  ...
  const files = pagesFiles.slice().filter(filename => {
    ...
    return file.dirname?.substring(0, dirname.length) === dirname && basename === '' || file.dirname === dirname && basename === sourceBasename;
  });
  const pageConfig = wrapMetadata(data)({
    filename: file.path
  }, files.reduce((a, b) => {
    const data = yaml.parse(readFileSync(b).toString());
    return wrapMetadata(data)({
      filename: b
    }, a);
  }, undefined)).page;
  const layouts = (pageConfig?.layout ?? []).map((layout, i) => ({
  ...
  const styles = Object.entries(pageConfig?.styles ?? {}).map(([name, style]) => ({
    type: 'ImportDeclaration',
    source: {
      type: 'Literal',
      value: style
  ...
  const _path = path.relative(path.resolve(process.cwd(), 'src', 'pages'), file.path).split(path.delimiter);
  const resource = {
    type: 'ObjectExpression',
    properties: [{
      type: 'Property',
            ...
            declarations: [{
              ...
              init: {
                ...
                arguments: [withOnce({
                  ...
                  body: {
                    type: 'BlockStatement',
                    body: [...Object.keys(pageConfig?.styles ?? {}).map(name => ({
          ...
          declaration: {
            ...
            arguments: [{
              ...
              body: {
                ...
                body: [{
                  type: 'ReturnStatement',
                  argument: (pageConfig?.layout ?? []).reduce((node, _, i) => ({
                    type: 'CallExpression',
                    callee: {
                      type: 'Identifier',
                      name: '__pages_jsx'
